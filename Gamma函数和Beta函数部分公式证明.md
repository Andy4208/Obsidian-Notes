...
$$ \Beta(p, q) = \int _0^1 t^{p-1} (1-t)^{q-1} \mathrm dt
$$
...
2. $\Beta(p, q) = \Beta(q, p)$
3. $\Beta(p + 1, q) = \dfrac{p}{p + q}\Beta(q, p)$
4. $\Beta(p, q) = \dfrac{\Gamma(p)\Gamma(q)}{\Gamma(p + q)}$
证明[^1]：
令 $t = \dfrac{x}{1 + x}$，有
$$ \Beta(p, q) = \int _0^\infty \dfrac{x^{p-1}}{(1+x)^{p+q}} \mathrm d x
$$
于是，
$$ \begin{align*}
    \Beta(p, q)\Gamma(p+q)
...
1. $\Beta(\alpha, 1-\alpha) = \Gamma(\alpha)\Gamma(1-\alpha) = \dfrac{\pi}{\sin \alpha\pi}$(余元公式)
证明[^2]：
由上述证明，
$$ \Beta(\alpha, 1-\alpha) = \int _0^\infty \dfrac{\mathrm d x}{x^\alpha (1+x)}
$$
...
$$ \begin{align*}
    ...
    &\to (1 - \mathrm e ^{-2\pi \alpha i})\Beta(\alpha, 1-\alpha)
\end{align*}
$$
因此
$$ \Beta(\alpha, 1-\alpha) = 
