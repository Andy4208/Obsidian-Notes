...
$\textbf{定义 1.}$ 函数列 $\{f_n(x)\}$ 一致收敛于 $f(x)$，若 $\forall \varepsilon > 0, \exist N > 0$，使得当 $n > N$ 时，$|f_n(x) - f(x)| < \varepsilon$.

$\textbf{定义 2.}$ 函数列 $\{f_n(x)\}$ 逐点收敛于 $f(x)$，若 $\forall \varepsilon > 0, \exist N(x) > 0$，使得当 $n > N$ 时，$|f_n(x) - f(x)| < \varepsilon$.
